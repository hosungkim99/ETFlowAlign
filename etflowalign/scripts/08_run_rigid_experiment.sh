...
if [ "$RUN_PREPARE" = "1" ]; then
  ...
  mkdir -p "$INPUT_DIR"
  "$PY" -m etflowalign.scripts.02_prepare_diffalign_example_train_batch \
    --repo-root "$REPO_ROOT" --out "$TRAIN_PT"
  "$PY" -m etflowalign.scripts.03_prepare_diffalign_example_batch \
...
"$PY" -m etflowalign.scripts.04_diagnose_vector_field \
...
for S in $TRAJ_SOLVERS; do
  "$PY" -m etflowalign.scripts.05_diagnose_sampling_trajectory \
    --checkpoint "$BEST_CKPT" \
    --input-batch "$INFER_PT" \
    --query-sdf "$QUERY_SDF" \
...
"$PY" -m etflowalign.scripts.06_export_candidates_to_sdf \
