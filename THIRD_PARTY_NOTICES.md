# Third-Party Notices

This repository includes or will include code copied from or adapted from the following projects.

## DiffAlign
- License: MIT
- Original copyright:
  - Copyright (c) 2022 Minkai Xu
  - Copyright (c) 2025 Iljung Kim
- License text: `third_party_license/DiffAlign_License`

## ET-Flow
- License: MIT
- Original copyright:
  - Copyright (c) 2024 Majdi Hassan, Nikhil Shenoy, Jungyoon Lee
- License text: `third_party_license/ET-Flow_License`


New code under `etflowalign` depends on and integrates these upstream components.
